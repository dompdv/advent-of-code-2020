defmodule AdventOfCode.Day10 do
  ...
  defp possibilities(acc, adapters, [h | rest]) do
    v = acc[h]
    acc = if MapSet.member?(adapters, h + 1), do: Map.put(acc, h + 1, v + Map.get(acc, h + 1)), else: acc
    acc = if MapSet.member?(adapters, h + 2), do: Map.put(acc, h + 2, v + Map.get(acc, h + 2)), else: acc
    acc = if MapSet.member?(adapters, h + 3), do: Map.put(acc, h + 3, v + Map.get(acc, h + 3)), else: acc
