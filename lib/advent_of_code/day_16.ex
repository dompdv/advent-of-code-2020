defmodule AdventOfCode.Day16 do
  ...
  def part2(args) do
    ...
    cleaned_nearby =
      Enum.filter(input.nearby, fn ticket ->
        Enum.all?(ticket, &valid_number?(&1, input.rules))
      end)
    ...
    sieved_fields =
      Enum.reduce(cleaned_nearby, hypo, fn ticket, h -> sieve(ticket, h, input.rules) end)
    ...
    depart_field =
      Enum.filter(solutions, fn {_index, m} ->
        [class] = MapSet.to_list(m)
  ...
  def converge(fields, mapping) do
    solved_field =
      Stream.drop_while(fields, fn {_index, m} -> MapSet.size(m) > 1 end) |> Enum.take(1)
    ...
    case solved_field do
      ...
      [{index, m}] ->
        new_mapping = Map.put(mapping, index, m)

        new_fields =
          Enum.map(Map.to_list(Map.delete(fields, index)), fn {i, h} ->
            {i, MapSet.difference(h, m)}
          end)
          |> Map.new()
  ...
  defp broken_rules(number, rules) do
    Enum.map(rules, fn {rule, ranges} ->
      {rule, Enum.any?(ranges, fn {l, h} -> l <= number and number <= h end)}
    end)
  ...
  defp parse_line(line, {:rules, acc}) do
    cond do
      ...
      true ->
        [class, rest] = String.split(line, ":")

        ranges =
          String.split(String.trim(rest), "or")
          |> Enum.map(&String.trim/1)
          |> Enum.map(fn s ->
            [l, h] = String.split(s, "-")
