defmodule AdventOfCode.Day17 do
  ...
  def part2(args) do
    active_cells = parse2(args)
    Enum.reduce(1..6, active_cells, &tick2/2)
    |> Enum.count()
  end

  defp activate2?({x, y, z, w} = cell, active_cells) do
    neighbors = Enum.sum(for dx <- -1..1, dy <- -1..1, dz <- -1..1, dw <- -1..1, {dx, dy, dz, dw} != {0,0,0,0}, do: if {x+dx, y+dy, z+dz, w+dw} in active_cells, do: 1, else: 0)
    cond do
      cell in active_cells and (neighbors == 2 or neighbors == 3) -> true
      (not (cell in active_cells)) and (neighbors == 3) -> true
      true -> false
  ...
  defp tick2(_, active_cells) do
    cell_to_consider2(MapSet.to_list(active_cells), MapSet.new())
    |> Enum.filter(fn cell -> activate2?(cell, active_cells) end)
    |> MapSet.new()
  end

  defp cell_to_consider2([], cells), do: cells
  defp cell_to_consider2([{x, y, z, w} | other], cells) do
    new_cells = for dx <- -1..1, dy <- -1..1, dz <- -1..1, dw <- -1..1, {dx, dy, dz, dw} != {0,0,0,0}, into: MapSet.new(), do: {x+dx, y+dy, z+dz, w+dw}
    cell_to_consider2(other, MapSet.union(cells, new_cells) )
  end

  defp build_coord_list2([], cells), do: cells
  defp build_coord_list2([{line, y} | rest], cells) do
    active_in_line =
      line |> String.graphemes() |> Enum.with_index()
      |> Enum.filter(fn {c, _i} -> c == "#" end)
      |> Enum.map(fn {_, i} -> {i, y, 0, 0} end)
      |> MapSet.new()
    build_coord_list2(rest, MapSet.union(cells, active_in_line))
  end
